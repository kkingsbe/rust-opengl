...
fn main() {
    ...
    //Locate executable path even if project is in workspace
    let executable_path = locate_target_dir_form_output_dir(&out_dir)
...
fn locate_target_dir_form_output_dir(mut target_dir_search: &Path) -> Option<&Path> {
    loop {
        //if path ends with "target", we assume it is the correct directory
        if target_dir_search.ends_with("target") {
            return Some(target_dir_search);
        }

        //Otherwise, keep going up until we find "target" dir
